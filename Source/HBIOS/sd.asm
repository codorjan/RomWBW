...
#ENDIF
;
#IF (SDMODE == SDMODE_MID)		; TP3465 MICROWIRE INTERFACE DEVICE (MID)
;
; MICROWIRE IS A PRECURSOR AND SUBSET OF SPI THAT HAPPENS TO BE
; COMPATIBLE WITH SD DEVICES. ASSUME 2 SD CARDS ARE CONNECTED
; (ALTHOUGH THE MID, AS WIRED, SUPPORTS 4 CHIP SELECTS).
;
SD_DEVCNT	.EQU	2		; ROMWBW SUPPORTS 2
SD_BASE		.EQU	$E0		; BASE ADDRESS
SD_FMB		.EQU	SD_BASE + $01	; FIRST MICROWIRE BYTE
SD_CSP		.EQU	SD_BASE + $0A	; CHIP SELECT PORT
SD_SKP		.EQU	SD_BASE + $0B	; CLOCK (SK) POLARITY
SD_MWM		.EQU	SD_BASE + $0C	; MICROWIRE MODE
SD_SKR		.EQU	SD_BASE + $0D	; CLOCK (SK) RATE
SD_ST		.EQU	SD_BASE + $0E	; STATUS
SD_PD		.EQU	SD_BASE + $0F	; PIN DEFINITION
SD_OPRREG	.EQU	SD_CSP
SD_OPRDEF	.EQU	%00000011	; BOTH /CS LINES DESELECTED
SD_CS0		.EQU	%00000001
SD_CS1		.EQU	%00000010
SD_PDDEF	.EQU	%11111100	; SET BOTH /CS PINS TO OUTPUT
SD_SKPDEF	.EQU	$00		; NORMAL CLOCK
SD_MWMDEF	.EQU	$00		; 8 DATA BITS
SD_SKRFAST	.EQU	%00000001	; CKIN/2 (MUST BE ADJUSTED SO MID INPUT CLOCK IS ALWAYS <= 5MHZ)
SD_SKRSLOW	.EQU	%00000111	; CKIN/128 (AS LONG AS IT'S <= ABOUT 400KHZ)
SD_IOBASE	.EQU	SD_BASE
#ENDIF
;
; SD CARD COMMANDS
;
...
;
#IF (SDMODE == SDMODE_MID)
	IN	A,(SD_ST)		; READ STATUS PORT
	AND	%01111111		; BITS OTHER THAN UWDONE SHOULD ALWAYS BE ZERO
	RET				; ZF SET IF SO
#ENDIF
;
#IF (SDMODE == SDMODE_DSD)
...
#IF (SDMODE == SDMODE_MID)
	; PER SPEC, THE CARD SHOULD NOW BE ABLE TO HANDLE FULL SPEED OPERATION
	; SET MID TO MAXIMUM SPEED
	CALL	SD_UWDONE		; MAKE SURE WE ARE DONE SENDING
	LD	A,SD_SKRFAST
	OUT	(SD_SKR),A
#ENDIF
;
...
#ENDIF
;
#IF ((SDMODE == SDMODE_MK4) | (SDMODE == SDMODE_DSD) | (SDMODE == SDMODE_PPI)| (SDMODE == SDMODE_MT) | (SDMODE == SDMODE_MID))
;
  #IF (SDMODE == SDMODE_MID)
	LD	A,SD_PDDEF
	OUT	(SD_PD),A		; SET /CS PINS TO OUTPUT
	LD	A,SD_SKPDEF
	OUT	(SD_SKP),A		; NORMAL CLOCK POLARITY (SPI MODE 0)
	LD	A,SD_MWMDEF
	OUT	(SD_MWM),A		; 8 DATA BITS
	LD	A,SD_SKRSLOW
	OUT	(SD_SKR),A		; SLOW CLOCK FOR INITIALIZATION
  #ENDIF
;
	LD	A,SD_OPRDEF
	LD	(SD_OPRVAL),A
...
#IF ((SDMODE == SDMODE_PPI) | (SDMODE == SDMODE_UART) | (SDMODE == SDMODE_SC) | (SDMODE == SDMODE_MID))
  #IF (SD_DEVCNT > 1)
	XOR	SD_CS0 | SD_CS1
  #ELSE
	XOR	SD_CS0
...
#IF (SDMODE == SDMODE_MID)
;
; MID: WAIT FOR END OF TRANSMISSION
;
SD_UWDONE:
	IN	A,(SD_ST)		; CHECK STATUS BYTE
	RLCA				; ROTATE BIT 7 INTO CARRY
	JR	NC,SD_UWDONE		; LOOP UNTIL SET
	RET
;
#ENDIF
;
...
SD_PUT:
;
#IF ((SDMODE == SDMODE_MT) | (SDMODE == SDMODE_MID))
;
  #IF (SDMODE == SDMODE_MT)
	OUT	(SD_WRTR),A
  #ENDIF
;
  #IF (SDMODE == SDMODE_MID)
	LD	C,A			; SAVE BYTE
	CALL	SD_UWDONE		; WAIT UNTIL WE ARE DONE SENDING
	LD	A,C			; GET BYTE BACK
	OUT	(SD_FMB),A		; WRITE TO MID
  #ENDIF
;
#ELSE
;
...
;
  #IF (SDMODE == SDMODE_MID)
	CALL	SD_UWDONE		; MAKE SURE WE ARE DONE SENDING
	LD	A,$FF
	OUT	(SD_FMB),A		; INITIATE TRANSFER
	CALL	SD_UWDONE		; WAIT FOR IT TO COMPLETE
	IN	A,(SD_FMB)		; GET BYTE RECEIVED
  #ENDIF
;
#ELSE
  #IF ((SDMODE == SDMODE_CSIO) | (SDMODE == SDMODE_MK4) | (SDMODE == SDMODE_SC))
